...
class Homographies:
    ...
    def calc_overlap(self, *k, vis=False, is_debug=False):
        img1 = self.images[k[0]].copy()
        img2 = self.images[k[1]].copy()
        kp1, _ = self.features[k[0]]
        kp2, _ = self.features[k[1]]
        good = self.matches[k]
        h, w = img1.shape
...
def test():
    ...
    #     print((homographies.images[imreader2.fpaths[i]] == imreader2[fpaths2[i]]).all())